...
/// Additive. Everything drawn into the paper buffer uses this, so a stroke
/// crossing another darkens rather than replacing it -- which is the whole
/// point of accumulating absorption instead of writing a colour.
...
/// A multiply in place: dst = src * dst. What the fade pass is drawn with, and
/// the reason the sheet is one buffer rather than a ping-pong pair. GL_ZERO for
/// the source factor discards the fragment's own colour except as the
/// multiplier, which is exactly the arithmetic wanted.
inline void setMultiplyBlend()
{
	glEnable( GL_BLEND );
